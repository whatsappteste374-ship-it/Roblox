...
local targetGameIds = {
    ...
    [85375890771250] = "KAT X",  -- Servidor alternativo
...
-- Função para debug - mostra o ID do jogo atual
local function DebugGameInfo()
    local currentGame = game:GetService("MarketplaceService"):GetProductInfo(gamePlaceId)
    LogMessage("ID do Jogo Atual: " .. gamePlaceId)
    LogMessage("Nome do Jogo: " .. currentGame.Name)
    ShowNotification("🔍 DEBUG", "ID: " .. gamePlaceId .. "\nNome: " .. currentGame.Name)
end

-- 🎯 BYPASS ESPECÍFICO PARA O JOGO KAT (111163066268338 E 85375890771250)
...
local function InitializeBypassSystem()
    ...
    -- Verifica se é algum jogo específico usando comparação numérica correta
    ...
    elseif gamePlaceId == 132352755769957 then
        ...
        LogMessage("Ativando proteção universal - Jogo não identificado")
        ShowNotification("🌐 MODO UNIVERSAL", "Jogo não identificado\nAtivando proteção universal...")
